...
def generateFallbackList(platform):
    ...
    for i in range(1 << len(p4)):
        ...
        if len(p) > 0:
            ret.append(p)
    return sorted(ret, key=cmp_to_key(fallbackCompare))
...
def updateCtx(conf, ctx, node):
    if "root" in node.attrib:
        ...
        v = node.attrib["root"].format(
            platform=ctx["platform"],
            config=os.path.join(ctx["config"], "config"),
            prjconfig=prjconfigdir,
        )
        f = node.attrib["root"].format(
            platform="{platform}",
            config=os.path.join(ctx["config"], "config"),
            prjconfig=prjconfigdir,
        )
        ...
        if not os.path.exists(ctx_root):
            ...
            for p in list:
                ...
                if os.path.exists(os.path.join(root, v)):
                    ...
                    list = []
        if (
            not os.path.exists(ctx_root)
            and "platforms" in conf
            and ctx["platform"] in conf["platforms"]
            and "family" in conf["platforms"][ctx["platform"]]
        ):
...
def parseLibrary(conf, node, ctx, seq_type):
    ...
    if (
        not (lib_name in ctx["externals"])
        or not ("external" in ctx["externals"][lib_name])
        or not (ctx["externals"][lib_name]["external"])
    ):
        for child in list(node):
            if child.tag == "source":
                parseFiles(
                    lib["source"]["files"], list(child), updateCtx(conf, ctx, child)
                )
...
def generateVCXPROJ(conf, ctx, lib, type):
    filename = os.path.abspath(
        "win32\\" + lib["name"] + "\\" + lib["name"] + ".vcxproj"
    )
            ...
            if dep_lib["name"] == dep and not isExcluded(conf, ctx, dep):
                for define in dep_lib["exports"]:
                    if dep_lib["exports"][define] is None:
                        ...
                        cpp_defs = (
                            cpp_defs + define + "=" + dep_lib["defines"][define] + ";"
                        )
    ...
    for dir in lib["header"]["includes"]:
        AdditionalIncludeDirectories = (
            AdditionalIncludeDirectories
            + os.path.relpath(os.path.abspath(dir), filepath)
            + ";"
        )
    dep_dirs = gatherIncludeDirs(conf, ctx, dep_closure)
    for dir in dep_dirs:
    ...
    if "" != lib["align"]:
        out.write(
            "<StructMemberAlignment>" + lib["align"] + "Byte</StructMemberAlignment>\n"
        )
    out.write("</ClCompile>\n")
    if "Application" == type:
        out.write("<Link>\n")
    ...
    for file in lib["header"]["files"]:
        if "h" == file["ext"]:
            out.write(
                '  <ClInclude Include="'
                + os.path.relpath(file["path"], filepath)
                + '"/>\n'
            )
    out.write("</ItemGroup>\n")
    out.write("<ItemGroup>\n")
...
def writeSourceRules(conf, ctx, lib_type, out, lib, build_dir):
    ...
    out.write("     \n")
    out.write(
        lib["name"] + "_includes=" + generateCPPFLAGS(conf, ctx, lib, build_dir) + "\n"
    )
    out.write("\n")
...
def generateLibraryMakefile(conf, ctx, lib_type, lib, filename, build_dir, src_dir):
    ...
    if lib_type == "static":
        out.write(
            "$(BUILD_DIR)/lib" + lib["name"] + ".a: $(objs_" + lib["name"] + ")\n"
        )
        out.write(
        ...
        out.write("\n")
    if lib_type == "shared":
        ld_deps = generateLDFlags(
            conf, ctx, lib_type, lib["deps"], [lib["name"]], True
        ) + generateExtLDFlags(conf, ctx)
        out.write(
            "$(BUILD_DIR)/lib"
            + lib["name"]
            + getSharedExt(conf, ctx)
            + ": $(objs_"
            + lib["name"]
            + ")\n"
        )
        if platformSubseteqTest(ctx["platform"], "darwin-*-gcc-*"):
            out.write(
                "\t$(CC) -dynamiclib -install_name lib"
                + lib["name"]
                + getSharedExt(conf, ctx)
                + " -o $(BUILD_DIR)/lib"
                + lib["name"]
            ...
            out.write(
                ...
                + lib["name"]
                + getSharedExt(conf, ctx)
                + " -Wl,-z,defs  -fPIC -o $(BUILD_DIR)/lib"
                + lib["name"]
                + getSharedExt(conf, ctx)
                + " $(objs_"
                + lib["name"]
            ...
            out.write(
                "\tstrip --remove-section=.comment --remove-section=.note --strip-unneeded $(BUILD_DIR)/lib"
                + lib["name"]
                + getSharedExt(conf, ctx)
            )
        out.write("\n")
    ...
    for dep in dep_dirs:
        ...
        if dep_dir in conf["format"]:
            ln_src = os.path.join(
                build_dir,
                "include",
                os.path.relpath(conf["format"][dep_dir], ctx["base"])
            ...
            if os.path.exists(ln_src):
                os.remove(ln_src)
            print(
                "symlink(" + ln_dst + ", " + ln_src + ")" + str(os.path.exists(ln_src))
            )
...
def generateToolMakefile(conf, ctx, lib_type, tool, filename, build_dir, src_dir):
    ...
    out.write(
        tool["name"]
        + "_ld="
        + generateLDFlags(conf, ctx, lib_type, tool["deps"], [], False)
        + "\n"
    )
    ...
    if lib_type == "static" and platformSubseteqTest(ctx["platform"], "linux-*-gcc-*"):
        static_flag = " --static"
    out.write(
        "\t$(CC) $(CFLAGS) $(CPPFLAGS) -o $(BUILD_DIR)/"
        + tool["name"]
        + " $(objs_"
        + tool["name"]
        + ") "
        + generateLDFlags(conf, ctx, lib_type, tool["deps"], [], True)
        + ext_ldflags
        + " $(LDFLAGS)\n"
    )
    out.write("\n")
    out.write("clean:\n")
    out.write(
        "\trm -f $(BUILD_DIR)/" + tool["name"] + " $(objs_" + tool["name"] + ")\n"
    )
...
def generateTypedMakefile(conf, ctx, lib_type):
    ...
    for tool in conf["tools"]:
        ...
        generateToolMakefile(
            conf, ctx, lib_type, tool, makefile, obj_dir, os.path.join("..", "..", "..")
        )
        deps = depClosure(conf, tool["deps"], True)
        out.write(tool["name"] + ":")
        for dep in deps:
        ...
        if ctx["externals"][ext_lib]["external"]:
            ...
            if "cppflags" in ctx["externals"][ext_lib]:
                out.write(
                    ...
                    + name
                    + '-cppflags="'
                    + ctx["externals"][ext_lib]["cppflags"]
                    + '"'
                )
    ...
    for lib in conf["libraries"]:
        if not lib["external"] and not isExcluded(conf, ctx, lib["name"]):
            ...
            if lib_type == "shared":
                out.write(
                    "\t@cp "
                    + os.path.join(obj_dir, lib_prefix + lib["name"] + lib_ext)
                    + " "
                    + target
                    + ".1\n"
                )
    ...
    for tool in conf["tools"]:
        if not isExcluded(conf, ctx, tool["name"]):
            ...
            out.write(
                "\t@cp "
                + os.path.join(obj_dir, tool["name"] + tool_ext)
                + " "
                + target
                + "\n"
            )
    ...
    out.write(
        "\t@cp "
        + os.path.join(ctx["base"], "config", "libopc.pc")
        + " "
        + target
        + "\n"
    )
...
def generateTypedMakefiles(ctx, source, lib_types):
    ...
    for lib_type in lib_types:
        for platform in ctx["platforms"]:
            ...
            out.write(
                "\t@$(MAKE) -C build" + os.sep + platform + os.sep + lib_type + "\n"
            )
    out.write("clean:\n")
    for lib_type in lib_types:
        for platform in ctx["platforms"]:
    ...
    for lib in conf["libraries"]:
        if not lib["external"] and not isExcluded(conf, ctx, lib["name"]):
            ...
            for lib_ext in lib_exts:
                ...
                zip.write(
                    os.path.join(obj_dir, lib_prefix + lib["name"] + lib_ext), target
                )
    for tool in conf["tools"]:
        if not isExcluded(conf, ctx, tool["name"]):
            target = base + "bin/" + tool["name"] + tool_ext
...
def installFiles(ctx, source, lib_type, base):
    ...
    copyFile(
        copy_ctx, os.path.join("third_party", "LICENSE"), base + "/LICENSE.THIRD_PARTY"
    )
...
if __name__ == "__main__":
    ...
    if 1 == len(ctx["platforms"]) and None != install_zip and 1 == len(includes):
        ...
        1 == len(ctx["platforms"])
        and platformSubseteqTest(ctx["platforms"][0], "win32-*-msvc-*")
        and 1 == len(includes)
    ):
        ctx["root"] = os.path.abspath(os.path.split(includes[0])[0])
